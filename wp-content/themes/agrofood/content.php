...
?>
<article <?php post_class( $post_class ) ?> data-index="<?php echo ( isset($args['index']) )?esc_attr( $args['index'] ):''; ?>">
	<?php if( $post_format != 'quote' ): ?>
		...
		if( !$tags_list ){
			$theme_options['ts_blog_tags'] = false;
		}
		?>
		<?php if( $theme_options['ts_blog_tags'] ): ?>
		...
		<div class="entry-content <?php echo !$show_blog_thumbnail?'no-featured-image':'' ?>">
			...
			<?php if( $theme_options['ts_blog_date'] || $theme_options['ts_blog_author'] || $theme_options['ts_blog_categories'] ): ?>
				<div class="entry-meta-bottom">
					...
					<?php if( $theme_options['ts_blog_date'] && 'product' != $post->post_type ) : ?>
					<!-- Blog Date Time -->
			...
			<?php if( $theme_options['ts_blog_excerpt'] ): ?>
				...
				<div class="entry-summary">
					...
					<?php
						if( $max_words != '-1' ){
							agrofood_the_excerpt_max_words($max_words, $post, $strip_tags, '', true);
						}
			...
			<?php if( $theme_options['ts_blog_read_more'] ): ?>
				...
				<div class="entry-meta-bottom-2">
					<a class="button-readmore button" href="<?php the_permalink() ; ?>"><?php esc_html_e('Read more', 'agrofood'); ?></a>
				</div>
			<?php endif; ?>
