<?php
/*** Comment ***/
function agrofood_list_comments( $comment, $args, $depth ){
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<div id="comment-<?php comment_ID(); ?>"  class="comment-wrapper">
			...
			<div class="comment-detail">
				...
				<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'agrofood' ); ?></em>
				<?php endif; ?>
				
				<div class="comment-meta">
					<span class="author"><?php echo sprintf( '<a href="%1$s" rel="external nofollow" class="url">%2$s</a>', get_comment_author_url(), get_comment_author() ); ?></span>
				...
				<div class="comment-actions">
					...
					<span class="edit"><?php edit_comment_link( esc_html__( 'Edit', 'agrofood' ), '' ); ?></span>
					<?php endif;?>
	...
	<li class="post pingback">
		<p><?php esc_html_e( 'Pingback:', 'agrofood' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_html__( '(Edit)', 'agrofood' ), ' ' ); ?></p>
	<?php
			break;
	endswitch;
}

function agrofood_comment_form( $args = array(), $post_id = null ){
	...
	$fields =  array(
		'author' => '<p class="author-row"><label>'.wp_kses( __('Your Name <span class="required">*</span>','agrofood'), $allowed_html ).'</label>' . '<input id="author" class="input-text" name="author" type="text" value="' .$comment_author. '" size="30"' . $aria_req . ' />' . '</p>'
		,'email'  => '<p class="email-row"><label>'.wp_kses( __('Your Email <span class="required">*</span>','agrofood'), $allowed_html ).'</label><input id="email" class="input-text" name="email" type="text" value="' . $comment_author_email . '" size="30"' . $aria_req . '/>' . '</p>'
	);

	$required_text = sprintf( ' ' . wp_kses( __('Required fields are marked %s','agrofood'), $allowed_html ), '<span class="required">*</span>' );
	$defaults = array(
		'fields'               => apply_filters( 'comment_form_default_fields', $fields )
		,'fields_before'		   => '<div class="info-wrapper">'
		,'fields_after'		   => '</div>'
		,'comment_field'        => '<div class="message-wrapper"><p><label>'.esc_html__('Your Comment', 'agrofood').'</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p></div>'
		,'must_log_in'          => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'agrofood' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>'
		,'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'agrofood' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>'
		,'comment_notes_before' => ''
		,'comment_notes_after'  => ''
		,'id_form'              => 'commentform'
		,'id_submit'            => 'submit'
		,'title_reply'          => esc_html__( 'Leave a comment', 'agrofood' )
		,'title_reply_to'       => esc_html__( 'Leave a Reply to %s', 'agrofood')
		,'cancel_reply_link'    => esc_html__( 'Cancel reply', 'agrofood' )
		,'label_submit'         => esc_html__( 'Submit', 'agrofood' )
	);
...
/* kses allowed html */
add_filter('wp_kses_allowed_html', 'agrofood_wp_kses_allowed_html', 10, 2);
function agrofood_wp_kses_allowed_html( $tags, $context ){
	switch( $context ){
		case 'agrofood_tgmpa':
			$tags = array(
...
/* Body classes filter */
add_filter('body_class', 'agrofood_body_classes_filter');
function agrofood_body_classes_filter( $classes ){
	...
	if( isset($theme_options['ts_enable_mobile_app_style']) && $theme_options['ts_enable_mobile_app_style'] ){
		$classes[] = 'mobile-app-style';
	}
	...
	if( $theme_options['ts_device_products_hide_compare_icon'] ){
		$classes[] = 'device-products-hide-compare-icon';
	}
