...
if( agrofood_is_active_filter_area() ){
	...
	$show_filter_area_by_default = agrofood_show_filter_area_by_default();

	if( $show_filter_area_by_default ){
		$filter_class[] = 'show-filter-default';
	}

	$filter_class[] = 'style-' . $theme_options['ts_filter_style'];
}

$extra_class = '';
$page_column_class = agrofood_page_layout_columns_class($theme_options['ts_prod_cat_layout']);
...
agrofood_breadcrumbs_title( $show_breadcrumb, $show_page_title, woocommerce_page_title(false) );
...
<div class="page-container <?php echo esc_attr($extra_class) ?>">
	...
	?>
	<div id="main-content" class="<?php echo esc_attr($page_column_class['main_class']); ?> <?php echo esc_attr(implode(' ', $filter_class)); ?>">	
		...
		<?php if ( woocommerce_product_loop() ) : ?>
			...
			if( class_exists('WC_Widget_Layered_Nav_Filters') ){
				...
				the_widget('WC_Widget_Layered_Nav_Filters', array('title' => esc_html__('Active filters:', 'agrofood')));
				echo '</div>';
			}
			?>
			...
			<div class="woocommerce main-products style-column-<?php echo esc_attr($theme_options['ts_prod_cat_layout_style']); ?> columns-<?php echo esc_attr($woocommerce_loop['columns']); ?>">
			<?php
