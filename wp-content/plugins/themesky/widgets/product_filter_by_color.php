		...
		if ( count($terms) > 0 ) {
			...
			foreach ( $terms as $term ) {
				...
				if( absint($datas['ts_color_image']) > 0  ){
					echo wp_get_attachment_image( absint($datas['ts_color_image']), 'ts_prod_color_thumb', true, array('title'=>$term->name, 'alt'=>$term->name) ) . "<span>{$term->name}</span>";
				}else{
					echo "<span style='background-color:{$datas['ts_color_color']}'></span><span>{$term->name}</span>";
				}					
